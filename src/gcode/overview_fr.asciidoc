...
[[cha:Vue-generale-G-code]] (((Vue générale du G-code de Machinekit)))
...
utilisé par Machinekit est basé sur le langage RS274/NGC. Cette documentation le 
...
puissance de ce langage et de Machinekit qui le met à profit. 
...
La persistance:: Quand Machinekit s'arrête, les paramètres volatiles perdent leurs
...
dans le fichier src/machinekit/rs274ngc/interp_array.cc .].
Les paramètres persistants sont enregistrés dans un fichier '.var' et restaurés à
leurs valeurs précédentes quand Machinekit est relancé. Les paramètres numérotés 
...
* '#<_vmajor>' - Version majeure de Machinekit. Si la version courante est 2.5.2,
2.5 est retourné.

* '#<_vminor>' - Version mineure du Machinekit. Si la version courante est 2.6.2,
0.2 est retourné.
...
interfaces graphiques TkMachinekit et Axis affichent tous les deux le programme G-code
...
des G-codes dans Machinekit est comprise entre 0 et 99. Toutefois, les valeurs
...
pas aux langage G-code de Machinekit.

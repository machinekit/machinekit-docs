...
* _axis_ - L'interface graphique AXIS pour Machinekit (The Enhanced Machine Controller).
* _axis-remote_ - Interface de télécommande d'AXIS.
* _comp_ - Crée, compile et installe des composants de HAL.
* _machinekit_ - LINUXCNC (The Enhanced Machine Controller).
* _gladevcp_ - Panneau de contrôle virtuel pour Machinekit, repose sur Glade, Gtk et
les widgets HAL.
...
* _halui_ - Observer des pins de HAL et commander Machinekit au travers d'NML.
...
utilisateur.
* _pyvcp_ - Panneau de Contrôle Virtuel pour Machinekit (Python Virtual Control Panel).
...
=== Composants du coeur de Machinekit[[sec:Realtime-Components-coeur]] ===
...
* _kins_ - (((kins)))Définition des cinématiques pour machinekit.

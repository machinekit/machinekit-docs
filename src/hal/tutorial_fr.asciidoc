...
temps qu' Machinekit depuis ses sources ou depuis un paquet. Si Machinekit a été
compilé en _run-in-place_, la man page n'est pas installée, mais elle
est accessible, dans le répertoire principal de Machinekit, taper:
----
...
Pour remédier à ces différences, l'équipe de Machinekit a proposé RTAPI, qui
fournit une manière cohérente aux programmes de parler au RTOS. Si vous
êtes un programmeur qui veut travailler à l'intérieur de Machinekit, vous
pouvez étudier _machinekit/src/rtapi/rtapi.h_ pour comprendre l'API. Mais si
...
machinekit/src. Si nécessaire, invoquez le script _rip-environment_ pour

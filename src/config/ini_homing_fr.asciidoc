...
internes de Machinekit en conséquence. Toutefois, les machines étant différentes
...
image::images/machinekit-motion-homing-diag_fr.png[]
...
contact d'origine machine. Une valeur différente de zéro indique à Machinekit
la présence d'un contact d'origine machine. Machinekit va alors commencer par
...
ce cas, Machinekit dégagera le contact à la vitesse définie ici. L'acquisition
...
Peut contenir les valeurs YES ou NO. Cette variable détermine si Machinekit
...
d'origine. Dans ce cas, Machinekit doit ignorer l'activation de cette limite
...
vraie (HOME_USE_INDEX = YES), Machinekit fera l'acquisition de l'origine
machine sur le premier front de l'impulsion d'index. Si elle est fausse
(=NO), Machinekit fera l'acquisition de l'origine sur le premier front produit
...
d'origine ou de l'impulsion d'origine, Machinekit ajuste les coordonnées de l'axe

...
l'ordinateur de Machinekit. footnote:[Cette section fait référence à 
l'utilisation de _stepgen_ le générateur de pas intégré à Machinekit. 
Certains dispositifs matériels ont leur propre générateur de pas 
et n'utilisent pas celui incorporé à Machinekit. Dans ce cas se référer 
...
logiciel Machinekit a quelques paramètres qui nous laissent régler 
les divers temps d'une période de différentes manières.

Par exemple, si _steplen_ footnote:[Steplen se réfère à un paramètre 
qui ajuste la performance du générateur de pas incorporé à Machinekit, 
...
en a la charge ? La réponse est que Machinekit peut contrôler ce système, 
mais il doit le faire en choisissant une des différentes méthodes 
de contrôle.

La méthode de contrôle qu'utilise Machinekit, est l'une des plus simples 
...
d'une consigne. Dans le cas de Machinekit le processus que nous 
...
Le noyau Machinekit (et probablement tous les noyaux RTAI-patché) ont les 
...
que votre système Machinekit peut éteindre le système par lui-même.

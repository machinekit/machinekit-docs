...
BASE_PERIOD est l'horloge de votre Machinekit. A chaque période, le
...
25µs, vous devez commencer à 25µs ou plus, lancer Machinekit et voir comment
...
31µs. Mais le générateur de pas logiciel de Machinekit a un certain nombre de
...
Open Office: 
http://wiki.machinekit.org/uploads/StepTimingCalculator.ods[Step Timing Calculator (en) - Calculatrice Calendrier étape (fr)].

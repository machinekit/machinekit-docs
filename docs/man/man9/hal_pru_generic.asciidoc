...
**"hpg.encoder.__<Instance>__.chan.__<Channel>__"**
...
*index-enable* (bit in/out):: When this pin is set to True, the count (and therefore also position) are reset to zero on the next Index (Phase-Z) pulse.  At the same time, index-enable is reset to zero to indicate that the pulse has occurred.

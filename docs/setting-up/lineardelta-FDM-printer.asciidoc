...
[NOTE]
more info about `HOMING` can be found in link:../config/ini_homing[]
...
then please visit the link:./index-HAL[HAL index]
with especially the link:../hal/intro[HAL intro] before continuing.

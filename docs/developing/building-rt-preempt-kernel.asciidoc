...
- Cut and Paste link:http://machinekit.io/docs/developing/rt-kernel-config[this] config to a file called .config in the root directory of the kernel sources. Meaning the `linux-3.18.11` directory that got created after unpacking. All the recent rt x86-amd64 patches up to kernel 4.1.5 have been tested and all worked well.
